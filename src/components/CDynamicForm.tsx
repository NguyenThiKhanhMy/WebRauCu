...
const CDynamicForm = forwardRef((props: Props, ref) => {      
    ...
    const CascaderRender = (propDef:IControlDefs) => {    
        let value = InitValues[propDef.Key];        
        let options:any = [];        
        let flag = true;
        if(props.options)
        ...
        return <>
            <CCascader disabled={propDef.IsDisabled} changeOnSelect={propDef.ChangeOnSelect} key={propDef.Key} filterable={propDef.Filterable} onChange={(e:any) => {onChangeValue(propDef.Key, e)}} value={value} options={options} keyOptions={propDef.KeyOptions} placeholder={propDef.Placeholder} />
            {RequiredControl(propDef, value)}
        </>
    } 
