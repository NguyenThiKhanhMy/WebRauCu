...
const CDynamicTable = forwardRef((props: Props, ref) => {      
    useEffect(() => {        
        ...
        {            
            ...
            {           
                ...
                if(columnDefs[j].Format)
                {
                    ...
                    else if(columnDefs[j].Format == "image")
                    {
                        value = '<img class="table-image cursor-pointer" src="'+ process.env.ROOT_URL + value +'" alt="image-thumbnail">';
                    }
                    else if(columnDefs[j].Format == "file")
                    {
                        // for(let)
                        let files = value;
                        value = '<div class="dropdown">'
                        value += '<span class="dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">';
                        value += '<i class="bi bi-file-earmark-fill"></i>';
                        value += '</span>';
                        value += '<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">';
                        let propKeyTitle = columnDefs[j].MappingProps.KeyTitle;
                        let propKeyURL = columnDefs[j].MappingProps.KeyURL;
                        for(let k = 0;k < files.length;k++)
                        {
                            let title = files[k][propKeyTitle];
                            let url = files[k][propKeyURL];                            
                            value += '<li><a class="dropdown-item" href="'+ (process.env.ROOT_URL + url) +'" target="_blank">'+ title +'</a></li> ';
                        }
                        value += '</ul">';
                        value += '</div">';
                    }
                    ...
                    {
                        value = value + columnDefs[j].Format
                    }
                }
                if(columnDefs[j].Key == "#")
                {
