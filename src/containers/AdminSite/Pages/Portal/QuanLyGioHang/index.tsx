...
const GioHangList = (props: Props) => {
  ...
  const [gioHangTaiKhoan, setgioHangTaiKhoan] = useState('');
  ...
  const ActionEvents = {
    onClickShow: () => {
      ...
      setgioHangTaiKhoan(getRowTaiKhoan());
    ...
    return (
      <>
        {dialogVisible == true ? (
          ...
          >
            <FormKichHoat ReloadTableItems={() => { Actions.GetItems(dispatch) }} Id={gioHangId} TaiKhoan={gioHangTaiKhoan} />
          </CDialog>
        ) : (
          <div></div>
        )}
  ...
  return (
    <>
      <CConfirm ref={refConfirm_DeleteItem} Title="Thao tác này sẽ xóa giỏ hàng này" Ok={async () => { await DeleteById() }} Canel={() => { }} />
