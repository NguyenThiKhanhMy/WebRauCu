...
export const Actions: any = {
  ...
  GetImportFile: async (
    ...
    let data = {
      IdKhoaHocDes: IdKhoaHocDes,
      IdKhoaHocSrc: IdKhoaHocSrc,
      IdGiaoAnSrc: IdGiaoAnSrc,
    };
    let res: IResponseMessage = await GiaoAnLyThuyet.GetMove(data);
    return res;
  },
  ...
  GetTreeList: async (dispatch: any) => {
    let res: IResponseMessage = await KhoaHocService.GetTreeList();
    dispatch({
      type: "GetTreeList",
      items: res.Data.Items,
    });
  },
  setURL_Anh: (item: any, dispatch: any) => {
    dispatch({
      type: "setURL_Anh",
      item: item,
    });
  ...
  DeleteTree: async (idKhoaHoc: any, dispatch: any) => {
    let res: IResponseMessage = await GiaoAnLyThuyet.DeleteTree(idKhoaHoc);
    return res;
  },
  CreateItemWithKhoaHoc: async (data: any, idKhoaHoc: any, dispatch: any) => {
    let res: IResponseMessage = await GiaoAnLyThuyet.CreateItemWithKhoaHoc(
      data,
      idKhoaHoc
    );
    if (res && res.Success) {
  ...
  CopyItem: async (idKhoaHoc: any, idGiaoAn: any, dispatch: any) => {
    let res: IResponseMessage = await GiaoAnLyThuyet.CopyItem(
      idKhoaHoc,
      idGiaoAn
    );
    return res;
