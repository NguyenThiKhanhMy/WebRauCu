...
const Comment = (props: Props) => {
  ...
  useEffect(() => {
    Actions.getCommentHocChinh(props.id, dispatch);
  }, []);
  ...
  const SaveMainComment = async () => {
    if (binhLuan.length > 0) {
      let res = await Actions.saveCommentHocChinh(
        props.id,
        userInfo.UserName,
        binhLuan
      );
      if (res && res.Success) {
        Actions.getCommentHocChinh(dispatch);
        setBinhLuan("");
      }
    }
    if (!binhLuan.length) {
  ...
  return (
    <div
      className="mt-2"
      style={{ backgroundColor: "white", padding: "8px 16px" }}
    >
      <CNotification ref={refNotification} />

      <h5 className="text-uppercase mt-2 mb-2 text-center">Bình luận</h5>
      <textarea
